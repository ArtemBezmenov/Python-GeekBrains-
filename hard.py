...
def copy():
         #Windows
         if os.name == 'nt':
             os.system(f'copy {dir_name} copy_{dir_name}')
         else:
         # Unix - OS
             os.system(f'cp {dir_name} copy_{dir_name}')
def remove():
    access = 0
    if not dir_name:
        print("Необходимо указать имя директории вторым параметром")
    ...
    dir_path = os.path.join(cur_dir, dir_name)
    if access != 1:
        confirm = input('Вы уверены, что хотите удалить файл? (y|n):')
        if confirm == 'y':
            access = 1
            try:
                os.rmdir(dir_path)
                ...
                print('Вы пытаетесь удалить файл!')
        else:
